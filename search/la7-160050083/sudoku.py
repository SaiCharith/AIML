...
class SudokuSearchProblem:
    ...
    def getSuccessors(self, state):
        ...
        ## YOUR CODE HERE
        s = -1
        possible_values = '9999999999999'
        for i in state:
            if(len(possible_values)>len(state[i]) and len(state[i])>1):
                possible_values = state[i]
                s = i
        for j in range(len(possible_values)):
            state_copy = state.copy()
            new_state=assign(state_copy,s,possible_values[j])
            # print(new_state)
            if new_state != False:
                successors.append((new_state,(s,possible_values[j]),1))
