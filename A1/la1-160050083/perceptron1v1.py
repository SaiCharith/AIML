	...
	def train( self, trainingData, trainingLabels, validationData, validationLabels, validate):
		...
		for iteration in range(self.max_iterations):
			...
			for i in range(len(trainingData)):
				...
				for ind in range(label1):
					label2 = self.legalLabels[ind]
					if self.weights[frozenset([label2, label1])] * trainingData[i] <= 0 :
						self.weights[frozenset([label2, label1])] += trainingData[i] 


				for ind in range(label1+1,len(self.legalLabels)):
					label2 = self.legalLabels[ind]
