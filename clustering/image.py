...
def read_image(filename):
	...
	with open(filename) as f:
		C = f.readline().strip()
		dims = f.readline()
		MaxPixelValue = int(f.readline())
		content = f.read()
	# C = content[0]
	W,H = dims.split()
	W = int(W) 
	H = int(H)
	# print H,len(content),filename,C
	
	if C=='P6':
		img = [[[0 for _ in range(3)] for _ in range(W)] for _ in range(H)]
		for i in range(H):
			for j in range(W):
				img[i][j][0] = ord(content[(i*W+j)*3])
				img[i][j][1] = ord(content[(i*W+j)*3+1])
				img[i][j][2] = ord(content[(i*W+j)*3+2])

	else:
		img = [[0 for _ in range(W)] for _ in range(H)]
		for i in range(H):
			for j in range(W):
				img[i][j] = ord(content[i*W+j])
...
def label_image(img, cluster_centroids):
	...
	for i in range(H):
		for j in range(W):
			distances = list(map(lambda p:distance(p,img[i][j]),cluster_centroids))
...
def write_image(filename, img):
	...
	if type(img[0][0])==type([]):
		f.write('P6\n')
		f.write(str(W)+" "+str(H)+"\n")
		Mx_value = max(list(map(lambda y: max(list(map(lambda x:max(x),y))),img)))
		Mx_value = min(int(Mx_value),255)
		f.write(str(Mx_value)+"\n")

		for i in range(H):
			for j in range(W):
				f.write(chr(min(255,int(img[i][j][0]))))
				f.write(chr(min(255,int(img[i][j][1]))))
				f.write(chr(min(255,int(img[i][j][2]))))

	else:
		f.write('P5\n')
		f.write(str(W)+" "+str(H)+"\n")
		Mx_value = max(list(map(lambda y: max(y),img)))
		Mx_value = min(int(Mx_value),255)
		f.write(str(Mx_value)+"\n")

		for i in range(H):
			for j in range(W):
				f.write(chr(min(255,int(img[i][j]))))
...
def decompress_image(cluster_labels, cluster_centroids):
	...
	for i in range(height):
		for j in range(width):
			l = (cluster_centroids[cluster_labels[i][j]])
			img[i][j][0] =int(l[0])
			img[i][j][1] =int(l[1])
			img[i][j][2] =int(l[2])
